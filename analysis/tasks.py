...
from analysis.framework import Task, GridWorkflow


class CreateChars(Task, GridWorkflow, law.LocalWorkflow):
    """
    ...
    By default, CreateChars is a grid (first workflow class in the inheritance order, MRO). If you
    want to execute it as a LocalWorkflow, add the ``"--workflow local"`` parameter on the command
    line. The code in this task should be completely independent of the actual *run location*, and
    law provides the means to do so.
    ...
    def output(self):
        ...
        return self.wlcg_target("output_{}.json".format(self.branch))
